...
$(document).ready(function() {
    ...
    function generateGanttBody() {
        const labelsContainer = $('#gantt-labels-body');
        const timelineContainer = $('#gantt-body');
        ...
        ganttData.workPackages.forEach(wp => {
            // Work Package row
            const { label, timeline } = generateWorkPackageRow(wp);
            ...
            if (showActivities && ganttData.activities[wp.id]) {
                ganttData.activities[wp.id].forEach(activity => {
                    const { label, timeline } = generateActivityRow(activity, wp);
                    labelsContainer.append(label);
                    timelineContainer.append(timeline);
                });
            }
        ...
        // Add empty row for bottom padding
        labelsContainer.append('<div class="gantt-row-padding"></div>');
        timelineContainer.append('<div class="gantt-row-padding"></div>');
    }
    ...
    function generateWorkPackageRow(wp) {
        ...
        const timeline = $(`
            <div class="gantt-row" data-wp-id="${wp.id}">
    ...
    function generateActivityRow(activity, wp) {
        ...
        if (!startDate || !endDate) {
            return { label: $(''), timeline: $('') }; // Skip activities without dates
        }
        ...
        const timeline = $(`
            <div class="gantt-row" data-activity-id="${activity.id}">
                <div class="gantt-timeline">
    ...
    function initializeControls() {
        // Sync scrolling between labels and timeline
        const labelsBody = $('#gantt-labels-body');
        const timelineBody = $('#gantt-body');

        labelsBody.on('scroll', function() {
            timelineBody.scrollTop($(this).scrollTop());
        });

        timelineBody.on('scroll', function() {
            labelsBody.scrollTop($(this).scrollTop());
        });
